...
class BookingItem {
  ...
  factory BookingItem.fromJson(Map<String, dynamic> json) {
    ...
    if (fullName.trim().isEmpty) fullName = 'You';
    ...
    return BookingItem(
      ...
      playgroundId: playground['_id']?.toString() ?? '',
...
class UserBookingApi {
  ...
  Future<List<BookingItem>> fetchBookings() async {
    ...
    print('➡️ [UserBookingApi] GET $uri');
    ...
    print('⬅️ [UserBookingApi] ${res.statusCode} ${res.body}');
  ...
  Future<BookingItem> updateBooking({
    ...
    final token = await _token();
    final uri = Uri.parse('$_baseUrl/api/v1/booking/update/$bookingId');
    ...
    print('➡️ [UserBookingApi] PUT $uri body=$body');
    ...
    final data = jsonDecode(res.body);
    if (res.statusCode >= 200 && res.statusCode < 300) {
      // your backend might return {data: booking}
      final updated = (data['data'] ?? data);
      return BookingItem.fromJson(updated);
    } else {
      final msg = data['message']?.toString() ?? 'Failed to update booking';
      throw Exception(msg);
    }
  }
}
